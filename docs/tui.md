...
`syncing`. A tick the machine slept through is not replayed — the next tick, or an `s`, runs the cycle. There is no
headless mode: without `--run-once` this UI is what runs, so leave it open in a `tmux` or `screen` window if you want it
to keep going after you close the terminal (see [Running it unattended](../README.md#running-it-unattended)).
