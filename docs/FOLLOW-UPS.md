...
- T29: nested bareRepoDirs across entries (`/b` and `/b/inner`) are not rejected; path comparison uses path.resolve without following symlinks, so two dirs aliased via symlink slip through (same limitation as the pre-existing checks); MCP startup with SYNC_WORKTREES_CONFIG logs a validation error to stderr and continues with no config loaded (pre-existing).
