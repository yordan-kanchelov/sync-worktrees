# Whole-application review for 6.0.0 (T1–T121)

**All 121 items are resolved: 120 implemented, 1 declined.** This began as a TODO list and is kept
as the record of what was reviewed and what was done about it. Each item still reads as the task
spec it was written as — location, current behaviour with its failure scenario, expected behaviour,
acceptance criteria — with its checkbox ticked by the commit that implemented it. See
[`6.0.0-plan.md`](6.0.0-plan.md) for how the work was run and [`FOLLOW-UPS.md`](FOLLOW-UPS.md) for
what was deliberately left out.

Review of sync-worktrees at commit `c7388d8` (v5.3.1): the worktree-mode sync
...
It continues [`REVIEW_FINDINGS.md`](REVIEW_FINDINGS.md) (F1–F20, all done) with `T`-numbered items. Each item is a
...
- [x] **T59** — No dry-run/plan surface: the planner is pure but there is no CLI or MCP way to
...
T52. **Product decisions**: settled -- T36 syncs on start by default (opt out via `defaults.syncOnStart`), T39 makes every MCP tool schema strict, T48 corrects the docs rather than widening hooks/file-copy to unattended syncs, T59 was declined for 6.0.0 and later shipped as `sync --dry-run`.
...
### [x] T59. No dry-run/plan surface

- **Decision**: first declined (trash makes a mistaken prune recoverable), then revisited after 7.1.0 and
  **fixed** as `sync --dry-run [--json]` (CLI only; no MCP tool). Every phase of the worktree-mode runner and the
  clone-mode tick was split into a read-only assessment and the mutations it asks for; the sync runs both, the dry run
  (`WorktreeSyncService.planSync`) runs only the assessments on a read-only GitService (`GIT_OPTIONAL_LOCKS=0`) and
  reports them. It fetches (remote-tracking refs only, documented) and writes nothing else. Covered by
  `sync-dry-run.e2e.test.ts` (plan contents, byte-identical repository state, and the following sync doing what the
  plan said) and `worktree-mode-sync-runner.plan.test.ts`. User docs: `docs/dry-run.md`.
- Original finding: the planner is pure but there is no CLI or MCP way to preview what a sync will create, prune or update before it mutates
