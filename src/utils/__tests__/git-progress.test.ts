...
describe("makeGitProgressHandler", () => {
  ...
  beforeEach(() => {
    infoSpy = vi.fn();
    debugSpy = vi.fn();
    logger = { info: infoSpy, warn: vi.fn(), error: vi.fn(), debug: debugSpy } as unknown as Logger;
  });
  ...
  it.each(["clone", "fetch", "pull"] as const)("emits debug logs for method '%s'", (method) => {
    ...
    expect(debugSpy).toHaveBeenCalledWith(expect.stringContaining(`${method} Receiving objects: 100%`));
  });
  ...
  it("always emits at 100% even if already emitted in the same bucket", () => {
    ...
    expect(debugSpy).toHaveBeenLastCalledWith(expect.stringContaining("100%"));
  });
  ...
  it("keeps default logs quiet while still forwarding structured progress", () => {
    ...
    expect(emitProgress).toHaveBeenCalledWith({
      phase: "fetch",
      message: "fetch Receiving objects: 25% (5/20)",
      progress: 25,
      processed: 5,
      total: 20,
    });
