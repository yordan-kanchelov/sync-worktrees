...
describe("GitService", () => {
    ...
    beforeEach(() => {
        ...
        (WorktreeMetadataService as jest.MockedClass<typeof WorktreeMetadataService>).mockImplementation(() => mockMetadataService as any);
    ...
    describe("initialize", () => {
        ...
        it("should resolve relative paths to absolute paths when creating worktrees", async () => {
            ...
            expect(mockGit.raw).toHaveBeenCalledWith(["worktree", "add", "--track", "-b", "main", expectedAbsolutePath, "origin/main"]);
        });
    ...
    describe("addWorktree", () => {
        ...
        it("should skip if directory is already a valid worktree", async () => {
            ...
            // Reset mockGit.raw and set up responses
            mockGit.raw.mockReset();
            mockGit.raw.mockResolvedValueOnce("worktree /test/worktrees/feature-1\n" + "HEAD abc123\n" + "branch refs/heads/feature-1\n\n"); // worktree list - shows the worktree exists
    ...
    describe("hasUpstreamGone", () => {
        ...
        it("should return false when upstream reference is ambiguous", async () => {
            ...
            expect(upstreamGone).toBe(false);
            expect(mockWorktreeGit.raw).toHaveBeenCalledWith(["rev-parse", "--abbrev-ref", "feat/autocue-frontend@{upstream}"]);
        });
...
branch refs/heads/feature-2`);
    ...
    describe("hasStashedChanges", () => {
        ...
        it("should return true when stash check fails", async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Error checking stash: Error: Failed to check stash"));
    ...
    describe("updateWorktree", () => {
        it("should update worktree and metadata for regular worktrees", async () => {
            ...
            expect(mockMetadataService.updateLastSync).toHaveBeenCalledWith(".bare/repo", "feature-1", "newcommit123", "updated");
        });
