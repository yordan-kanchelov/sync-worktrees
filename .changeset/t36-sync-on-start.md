...
Because the startup cycle and the cron jobs are armed in the same breath, a tick landing inside the startup sync went from a rare `s`-during-a-sync to something every run can meet. The UI now runs one sync cycle at a time: a tick (or an `s`) that arrives while one is in flight logs that a sync is already running and skips. The per-repository lock always stopped the second cycle doing any real work, but not before it had cleared each service's recorded clone-mode skips out from under the running one and driven the status bar back to idle mid-sync.
